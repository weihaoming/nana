...
public:
	...
	FRotator GetRotationToCrosshairDirection(const FVector& From, FName TraceProfileName, float MinDistanceToRotateToTarget = 100.f, float Distance = 100000.f) const;

	UFUNCTION(BlueprintPure, Category = "AimAssistComponent")
	const FVector2D& GetCrosshairPosition() const { return CrosshairPosition; }

	UFUNCTION(BlueprintPure, Category = "AimAssistComponent")
	bool GetCurrentTargetSocketLocation(FVector& OutLocation) const;

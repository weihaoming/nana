...
void UUASAimAssistComponent::BeginPlay()
{
	...
	if (PlayerController.IsValid())
	{
		...
		FViewport::ViewportResizedEvent.AddUObject(this, &UUASAimAssistComponent::OnViewportResized);
...
FRotator UUASAimAssistComponent::GetRotationToCrosshairDirection(const FVector& From, FName TraceProfileName, float MinDistanceToRotateToTarget, float Distance) const
{
	if (PlayerController.IsValid())
	{
		FVector worldOrigin;
		...
		if (UGameplayStatics::DeprojectScreenToWorld(PlayerController.Get(), crosshairPosition, worldOrigin, worldDirection))
		{
			...
			if (CurrentTargetData.IsValid() && bMagnetismAreaActive && (CurrentTargetData.GetSocketLocation() - From).Size() >= MinDistanceToRotateToTarget)
			{
...
bool UUASAimAssistComponent::GetCurrentTargetSocketLocation(FVector& OutLocation) const
{
	if (CurrentTargetData.IsValid())
	{
		OutLocation = CurrentTargetData.GetSocketLocation();
		return true;
	}
...
void UUASAimAssistComponent::PrintSettings()
{
	UE_LOG(LogTemp, Warning, TEXT("Print All Settings!1112"))
}
...
void UUASAimAssistComponent::HandleAutoAim(float DeltaTime)
{
	...
	if (!CurrentTargetData.IsValid()
		...
		|| PlayerController->GetPawn()->GetMovementComponent() == nullptr
		|| !bAutoAimAreaActive
		|| !IsAutoAimEnabled())
	{
...
{
	if (!PlayerController.IsValid() || !IsScalingEnabled() || PlayerController->GetPawn() == nullptr)
	{
		return;
	}
...
{
	if (!CurrentTargetData.IsValid() || !PlayerController.IsValid() || !IsStickinessEnabled() || !bStickinessAreaActive || PlayerController->GetPawn() == nullptr)
	{
		Pitch = 1.f;
		Yaw = 1.f;
		return;
...
void UUASAimAssistComponent::OnViewportResized(FViewport* Viewport, uint32)
{
	if (PlayerController.IsValid() && GetWorld() != nullptr && GetWorld()->GetGameViewport() && GetWorld()->GetGameViewport()->Viewport == Viewport && AimAssistDataAsset != nullptr)
	{
